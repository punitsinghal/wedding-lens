...
**Notable friction in this flow:**
- ~~**Event detail page carries seven distinct jobs on one screen**: analytics, publish/consent, guest-access revoke, cover photo, event-details form, photographer management, danger zone. There's no tab or section nav — it's one long scroll...~~ **Fixed:** the page is now split into four tabs — **Overview** (analytics, event identity: name/bride/groom/date/slug), **Publish & Access** (cover photo, guest access mode/code/OTP, publish/consent, guest-access revoke), **Photographers**, and **Danger Zone** (owner-only tabs hidden entirely for assigned photographers, who only see Overview and Publish & Access read-only) — `frontend/app/events/[eventId]/page.tsx`. Cover photo now sits *above* the Publish button in the same tab, so the "set a cover first" hint refers to something the owner has already seen instead of a section further down the page.
- ~~**Confirmed gap:** the publish button copy says a cover photo is "**Required to publish**," ... the button's client-side `disabled` logic only checks the consent checkbox and event status, not `cover_photo_id`.~~ **Fixed:** the Publish button's `disabled` logic now also checks `event.cover_photo_id`, with a matching amber hint ("Set a cover photo (above) to enable the Publish button.") next to the existing consent hint — `frontend/app/events/[eventId]/page.tsx`.
...
| Guest returns to Favourites after 24h idle or a server restart | Empty favourites list; a standing notice on the page now sets the expectation that favourites aren't permanent | `FavouritesStore` is in-memory with a 24h sliding TTL; state does not survive a restart (store itself unchanged — UI now discloses the limitation) |
| Owner clicks Publish with no cover photo set | Publish button is disabled with an amber hint to set a cover photo first | Frontend now checks `cover_photo_id` client-side, matching the backend's REQ-31 validation, so the round-trip failure no longer occurs |
...
- [x] ~~Should the cover-photo requirement to publish be enforced in the UI's disabled-button logic, or does the backend already block it?~~ **Fixed:** the Publish button now checks `event.cover_photo_id` client-side, matching the backend's REQ-31/AC-19 enforcement — `frontend/app/events/[eventId]/page.tsx`.
...
- [x] ~~Should the Favourites page warn guests that favourites aren't durable, or should the store move to persistent backing?~~ **Decided (interim): warn, don't migrate.** Added a notice on the Favourites page; `FavouritesStore` stays in-memory per the existing ADR. Revisit moving to a persistent backing (e.g. Postgres) as a separate ADR-worthy decision if guest volume outgrows the single-VM MVP assumption — owner: product/engineering
...
- [x] ~~Should the event detail page be split into tabs/sections (Overview / Publish & Access / Photographers / Danger Zone) now, or is single-scroll acceptable?~~ **Done.** Split into exactly those four tabs — `frontend/app/events/[eventId]/page.tsx`.
