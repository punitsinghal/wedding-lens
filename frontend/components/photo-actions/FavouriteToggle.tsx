...
export default function FavouriteToggle({ isFavourited, onToggle, className = '' }: FavouriteToggleProps) {
  return (
    <button
      type="button"
      onClick={(e) => { e.stopPropagation(); onToggle(); }}
      aria-label={isFavourited ? 'Remove from favourites' : 'Add to favourites'}
      className={`p-1.5 rounded-full bg-black/40 hover:bg-black/60 transition-colors ${className}`}
    >
      {isFavourited ? (
        // Filled heart
